...
import { 
  ...
  IconButton, FormGroup, FormControlLabel, Checkbox, Button,
  InputAdornment, Chip
} from '@mui/material';
...
export default function StayDetails() {
  ...
  const handleRemoveAmenity = (hotelId, amenityToRemove) => {
    const hotel = hotels.find(h => h.id === hotelId);
    const safeAmenities = hotel.amenities || []; // Safety check
    handleUpdate(hotelId, 'amenities', safeAmenities.filter(a => a !== amenityToRemove));
  };
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 4 }, maxWidth: 1200, mx: 'auto', bgcolor: '#f8fafc', minHeight: '100vh', pb: 12 }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 3 }}>
        <Box>
          <Typography variant="h5" fontWeight="800" color="#0f172a" mb={0.5}>Hotel Details</Typography>
      ...
      {(hotels || []).map((hotel, index) => {
        ...
        return (
          <Paper key={hotel.id} elevation={0} sx={{ p: 4, border: '1px solid #e2e8f0', borderRadius: '12px', mb: 3, bgcolor: '#fff' }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
              ...
              {index > 0 && (
                <IconButton size="small" onClick={() => handleRemoveHotel(hotel.id)} sx={{ color: '#ef4444', border: '1px solid #fecaca', bgcolor: '#fff', borderRadius: '6px' }}>
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} md={3}>
                ...
                <StyledTextField 
                  select fullWidth 
                  value={hotel.hotelPref || 'Luxury Resort'} onChange={(e) => handleUpdate(hotel.id, 'hotelPref', e.target.value)}
                >
              ...
              <Grid item xs={12} md={3}>
                 ...
                 <StyledTextField 
                   select fullWidth 
                   value={hotel.checkInTime || '3:00 PM'} onChange={(e) => handleUpdate(hotel.id, 'checkInTime', e.target.value)}
                 >
              ...
              <Grid item xs={12} md={3}>
                 ...
                 <StyledTextField 
                   select fullWidth 
                   value={hotel.checkOutTime || '11:00 AM'} onChange={(e) => handleUpdate(hotel.id, 'checkOutTime', e.target.value)}
                 >
              ...
              <Grid item xs={12} md={3}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  <IconButton size="small" onClick={() => handleCountUpdate(hotel.id, 'rooms', -1)} sx={{ border: '1px solid #e2e8f0', borderRadius: '6px', width: 36, height: 36 }}>
                    <Remove fontSize="small" sx={{ color: '#64748b' }} />
                  </IconButton>
                  <Box sx={{ border: '1px solid #e2e8f0', width: '100%', height: 36, borderRadius: '6px', display: 'flex', alignItems: 'center', px: 2 }}>
                    <Typography variant="body2" sx={{ fontWeight: 500, color: '#334155' }}>{hotel.rooms || 1}</Typography>
                  </Box>
                  <IconButton size="small" onClick={() => handleCountUpdate(hotel.id, 'rooms', 1)} sx={{ border: '1px solid #e2e8f0', borderRadius: '6px', width: 36, height: 36 }}>
              ...
              <Grid item xs={12} md={3}>
                ...
                <StyledTextField 
                  ...
                  placeholder="200"
                  value={hotel.price || ''} 
                  onChange={(e) => handleUpdate(hotel.id, 'price', e.target.value)}
                  InputProps={{
                    startAdornment: <InputAdornment position="start" sx={{ color: '#475569' }}>$</InputAdornment>,
                  }}
                />
              </Grid>
              ...
              <Grid item xs={12} md={3}>
                ...
                <Box sx={{
                  ...
                  {safeAmenities.length === 0 && (
                    <Typography variant="body2" sx={{ color: '#94a3b8', ml: 1 }}>No amenities added</Typography>
                  )}
                </Box>
              </Grid>
              ...
              <Grid item xs={12}>
                ...
                <FormGroup row sx={{ mt: -0.5, gap: 2 }}>
                  <FormControlLabel control={<Checkbox checked={hotel.meals?.breakfast || false} onChange={(e) => handleMealUpdate(hotel.id, 'breakfast', e.target.checked)} size="small" sx={{ '&.Mui-checked': { color: '#0ea5e9' } }} />} label={<Typography variant="body2" color="#334155" fontWeight="500">Breakfast</Typography>} />
                  <FormControlLabel control={<Checkbox checked={hotel.meals?.lunch || false} onChange={(e) => handleMealUpdate(hotel.id, 'lunch', e.target.checked)} size="small" sx={{ '&.Mui-checked': { color: '#0ea5e9' } }} />} label={<Typography variant="body2" color="#334155" fontWeight="500">Lunch</Typography>} />
                  <FormControlLabel control={<Checkbox checked={hotel.meals?.dinner || false} onChange={(e) => handleMealUpdate(hotel.id, 'dinner', e.target.checked)} size="small" sx={{ '&.Mui-checked': { color: '#0ea5e9' } }} />} label={<Typography variant="body2" color="#334155" fontWeight="500">Dinner</Typography>} />
                  <FormControlLabel control={<Checkbox checked={hotel.meals?.allInclusive || false} onChange={(e) => handleMealUpdate(hotel.id, 'allInclusive', e.target.checked)} size="small" sx={{ '&.Mui-checked': { color: '#0ea5e9' } }} />} label={<Typography variant="body2" color="#334155" fontWeight="500">All Inclusive</Typography>} />
                </FormGroup>
              </Grid>
            </Grid>
          </Paper>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 1 }}>
        <Box sx={{ display: 'flex', gap: 2 }}>
          <Button 
            variant="outlined" size="small" startIcon={<Add sx={{ fontSize: 18 }} />} 
            onClick={handleAddHotel} 
            sx={{ color: '#334155', borderColor: '#e2e8f0', fontWeight: 600, textTransform: 'none', borderRadius: '8px', px: 2, bgcolor: '#fff' }}
          >
            Hotel
          </Button>
          <Button 
        ...
        <Button 
          variant="outlined" size="small" 
          sx={{ color: '#334155', borderColor: '#e2e8f0', fontWeight: 600, textTransform: 'none', borderRadius: '8px', px: 3, bgcolor: '#fff' }}
        >
          Add-Ons
        </Button>
      </Box>
