...
import { 
  ...
  ListItemIcon, ListItemText, Avatar, InputBase, Divider 
} from '@mui/material';
import { 
  GridView, SupportAgent, Map, Settings, Search, TravelExplore 
...
export default function Sidebar() {
  ...
  const menuItems = [
    ...
    { title: 'Lead Management', icon: <SupportAgent />, path: '/lead-management' }, // <-- NEW SECTION
  ...
  return (
    <Box sx={{ 
      ...
      <Box sx={{ p: 3, display: 'flex', alignItems: 'center', gap: 1.5 }}>
        <Box sx={{ width: 32, height: 32, bgcolor: '#00c6ff', borderRadius: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      {/* 2. SEARCH BAR */}
      <Box sx={{ px: 3, mb: 3 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', bgcolor: '#f1f5f9', borderRadius: 2, px: 2, py: 1 }}>
      ...
      <Box sx={{ mt: 'auto' }}>
        ...
        <Box sx={{ p: 3, display: 'flex', alignItems: 'center', gap: 2 }}>
          <Avatar sx={{ width: 40, height: 40, bgcolor: '#00c6ff', color: '#fff', fontWeight: 800 }}>
            J
          </Avatar>
          <Box>
